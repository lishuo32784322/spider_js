            ...
            if (!(o.t <= 0)) {
                ...
                if (0 != p) {
                    ...
                    for (s.dlShiftTo(v, w),
                        ...
                        s[s.t++] = 0;
                    for (; --v >= 0; ) {
                        var b = n[--M] == p ? this.DM : Math.floor(n[M] * g + (n[M - 1] + m) * y);
                        if ((n[M] += s.am(0, b, n, v, 0, f)) < b)
                            for (s.dlShiftTo(v, w),
                                     n.subTo(w, n); n[M] < --b; )
                                n.subTo(w, n)
                    }
...
function get_pwd(pwd) {
    ...
    var e = RSA;
    e.prototype.setPublic(rsaModulus, rsaExponent);
    return e.prototype.encrypt(pwd)
}